class MedMateApp {
    ...
    initializeApp() {
        ...
        console.log('MedMate App initialized for mobile!');
    }
    ...
    initializeTabs() {
        // Initial tab is set to dashboard in HTML
    }
    ...
    initializeTheory() {
        ...
        document.querySelectorAll('.start-exam[data-exam="theory"]').forEach(card => {
    ...
    finishQuiz() {
        ...
        this.userStats.theories.completed += this.quizState.questions.length;
        this.userStats.theories.correct += correctAnswers;
        this.currentXP += correctAnswers * 10;
        
        this.dailyProgress.questions += this.quizState.questions.length;
        this.updateDailyProgress();
        
        this.showDetailedQuizResults();
        
        this.showToast(`Тест завершен! Правильных ответов: ${correctAnswers}/${this.quizState.questions.length}`, 'success');
        this.updateAllDisplays();
    }
    ...
    showDetailedQuizResults() {
        ...
        const correctAnswers = this.quizState.userAnswers.filter((answer, index) => 
            answer !== -1 && this.quizState.questions[index].options[answer].correct).length;
        ...
        let resultsHTML = `
            <div class="results-card">
                ...
                <div class="results-stats">
                    ...
                    <div class="result-stat">
                        <div class="stat-value">${this.quizState.questions.length}</div>
                        <div class="stat-label">Всего</div>
                    </div>
                    <div class="result-stat">
                        <div class="stat-value">${Math.round((correctAnswers / this.quizState.questions.length) * 100)}%</div>
                ...
                <div class="results-xp">
                    ...
                    Получено <strong>${correctAnswers * 10}</strong> XP
                </div>

                <div class="detailed-results">
                    <h4 style="margin-bottom: 16px; text-align: left;">Детальные результаты:</h4>
        `;
            ...
            resultsHTML += `
                <div class="question-result ${isCorrect ? 'correct' : 'incorrect'}">
                    <div style="display: flex; justify-content: between; align-items: flex-start; margin-bottom: 12px;">
                        <h5 style="margin: 0; flex: 1; font-size: 14px;">Вопрос ${index + 1}: ${question.question}</h5>
        ...
        resultsHTML += `
                ...
                <button class="button primary" id="restartQuiz" style="margin-top: 16px;">
        ...
        this.getElement('#restartQuiz')?.addEventListener('click', () => {
            this.startRandomQuiz();
        });
    ...
    initializeStations() {
        ...
        document.querySelectorAll('.start-exam[data-exam="stations"]').forEach(card => {
            card.addEventListener('click', (e) => {
                e.preventDefault();
                ...
                <div class="modal-body">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin: 16px 0;">
                        ...
                        <div class="scoring-section">
                            ...
                            <button class="button primary" id="submitStation" style="width: 100%; margin-top: 8px;">Завершить станцию</button>
                        </div>
                    </div>
    ...
    initializeTasks() {
        ...
        document.querySelectorAll('.start-exam[data-exam="tasks"]').forEach(card => {
            card.addEventListener('click', (e) => {
                e.preventDefault();
        ...
        modal.innerHTML = `
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="task-actions" style="display: flex; gap: 12px; margin-top: 20px;">
    ...
    displayTaskQuestion(index) {
        ...
        if (Array.isArray(question.correct)) {
            optionsHTML = question.options.map((option, optIndex) => {
                ...
                return `
                    <label class="task-option">
                        <input type="checkbox" name="question-${index}" value="${optIndex}" ${isChecked ? 'checked' : ''}>
                        <span>${option}</span>
                    </label>
                `;
            }).join('');
            ...
            optionsHTML = question.options.map((option, optIndex) => {
                ...
                return `
                    <label class="task-option">
                        <input type="radio" name="question-${index}" value="${optIndex}" ${isChecked ? 'checked' : ''}>
                        <span>${option}</span>
                    </label>
                `;
            }).join('');
        ...
        container.innerHTML = `
            <div class="task-question">
                <h4>Вопрос ${index + 1} из ${this.currentTask.questions.length}</h4>
                <p>${question.question}</p>
                <div class="task-options">
                    ${optionsHTML}
                </div>
                <div id="taskResults-${index}" class="task-results-immediate" style="display: none;"></div>
            </div>
        `;
        ...
        if (Array.isArray(question.correct)) {
            ...
            checkboxes.forEach(checkbox => {
                checkbox.addEventListener('change', () => {
                    ...
                    this.showSelectedAnalysis(questionIndex);
                });
            });
        } else {
    ...
    showSelectedAnalysis(questionIndex) {
        const container = this.currentTask.modal.querySelector('#taskQuestionsContainer');
        const question = this.currentTask.questions[questionIndex];
        const resultsContainer = container.querySelector(`#taskResults-${questionIndex}`);
        
        const userAnswer = this.currentTask.userAnswers[questionIndex];
        if (userAnswer !== null) {
            let selectedOptions = [];
            
            if (Array.isArray(userAnswer)) {
                selectedOptions = userAnswer.map(index => question.options[index].text);
            } else {
                selectedOptions = [question.options[userAnswer].text];
            }
            
            resultsContainer.style.display = 'block';
            resultsContainer.innerHTML = `
                <h6><i class="fas fa-file-medical-alt"></i> Результаты выбранного обследования</h6>
                <p>Вы выбрали: ${selectedOptions.join(', ')}</p>
    ...
    finishTask() {
        ...
        this.currentTask.modal.remove();
        this.showToast(`Задача завершена! Правильных ответов: ${correctAnswers}/${this.currentTask.questions.length}`, 'success');
        this.updateAllDisplays();
    }

    // Quick Actions
    initializeQuickActions() {
        this.getElement('#quickTheory')?.addEventListener('click', (e) => {
            e.preventDefault();
            this.startRandomQuiz();
        });

        this.getElement('#quickStation')?.addEventListener('click', (e) => {
            e.preventDefault();
            this.startHistoryStation();
        });

        this.getElement('#quickTask')?.addEventListener('click', (e) => {
            e.preventDefault();
            this.startCOPDTask();
        });

        this.getElement('#quickProgress')?.addEventListener('click', (e) => {
            e.preventDefault();
